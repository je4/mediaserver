...
import (
	...
	"github.com/je4/mediaserver/v2/pkg/api/docs" // generated by swag cli
...
func NewController(conf *config.ApiConfig, cert *tls.Certificate, dbClient *databasePG.Client) (*controller, error) {
	...
	u, err := url.Parse(conf.ExtAddr)
	if err != nil {
		return nil, errors.Wrapf(err, "invalid external address '%s'", conf.ExtAddr)
	}
	...
	router := gin.Default()
	c := &controller{addr: conf.Addr, router: router, subpath: subpath, dbClient: dbClient}
...
func (ctrl *controller) Init(cert *tls.Certificate) error {
	var auth = func() gin.HandlerFunc {
		return func(c *gin.Context) {
			if len(c.GetHeader("Authorization")) == 0 {
				NewResultMessage(c, http.StatusUnauthorized, errors.New("Authorization is required Header"))
				c.Abort()
			}
			c.Next()
		}
	...
	v1.Use(auth())
	v1.PUT("/:collection", ctrl.newMediaItem)
...
// @Param	     NewMediaRequest	body		NewMediaItemRequestBody	true	"Add new media"
...
func (ctrl *controller) newMediaItem(c *gin.Context) {
	collection := c.Param("collection")
	_ = collection
	var requestBody NewMediaItemRequestBody
